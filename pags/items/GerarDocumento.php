...
<div class="w3-panel w3-padding w3-right">
    <a href="<?=URL::getBase();?>Pagina/Painel/<?=$nomeUsuario; ?>" ><button class="w3-button w3-red">VOLTAR</button></a>
</div>
...
for ($i=0; $i < $cont; $i++) { 
    if (isset($dados['nomes']) && $dados['nomes'][$i] != "") {
        $len = isset($dados['id']) ? count($dados['id']) : 0;

        if ($len == 0) {
            $sql = "INSERT INTO planilhas (Nome, Endereco, Whatsapp, Valor) VALUES (?,?,?,?)";
            $pdo->prepare($sql)->execute([$dados['nomes'][$i], $dados["endereco"][$i], $dados['whatsapp'][$i], $dados['divida'][$i]]);
        } else {
            $id = isset($dados['id'][$i]) ? $dados['id'][$i] : 0;

            $buscar = $pdo->prepare("SELECT * FROM planilhas WHERE ID=?");
            $buscar->execute([$id]);
            
            $resultado = $buscar->fetch();
        
            if ($resultado) {
                $sql = "UPDATE planilhas SET Nome=?, Endereco=?, Whatsapp=?, Valor=?  WHERE ID=?";
                $pdo->prepare($sql)->execute([$dados['nomes'][$i], $dados["endereco"][$i], $dados['whatsapp'][$i], $dados['divida'][$i], $dados['id'][$i]]); 
